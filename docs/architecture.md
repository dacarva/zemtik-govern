...
## Output Seam (post-invocation rail, #39/#40)

### Scope: proxy() only

The output seam runs **only inside `proxy()`**. Direct `govern()` /
`govern_sync()` callers are input-only — no output screening or redaction occurs
on those paths. The non-keyed `proxy()` path gets output screening but NOT
effect-idempotency.

### How it works

After the governed tool returns, `proxy()` calls `_screen_output()`, which:

1. Projects the return value to screened text (`str`, `bytes`, JSON-native; any
   other type is denied fail-closed — an unscreenable value is never passed
   through).
2. Applies NFKC normalisation to the projected text (closes the Unicode
   compatibility-form bypass).
3. Runs each configured `OutputClassifier` in order.

On a match the outcome depends on the tool's I/O classification:

| I/O class | Rail mode | Outcome | Audit event | Severity |
|-----------|-----------|---------|-------------|----------|
| `read` | `enforce` | `OutputGovernanceDenied` raised; value withheld | `output_denied_raised` | — |
| `write` | `enforce` | `RedactedOutput` sentinel returned; value scrubbed | `output_denied_redacted` | HIGH |
| either | `shadow` | Value returned unblocked; would-deny observed | `output_would_deny` | — |
| either | — | Clean output passes through | `output_allowed` | — |

### The output-deny asymmetry

The asymmetry is deliberate. A read tool's output can be withheld — the effect
is the query, not the response. A write tool has already mutated external state
by the time the response arrives; raising instead of returning would lose the
side-effect record without undoing the mutation. Returning `RedactedOutput` lets
the caller log and track the event without receiving the offending value.

### RedactedOutput sentinel contract

`RedactedOutput` has two halves:

- **Spare** — `str()`, `repr()`, `format()` return `"<output redacted: audit_id=…>"`.
  Structured loggers that call `default=str` never crash on the sentinel.
- **Poison** — attribute access (other than `.audit_id`), item access, and
  iteration raise `RedactedOutputAccessError`. A caller that accidentally tries
  to use the value is loudly signalled.

`gov.unwrap(result)` collapses the asymmetry: it raises `OutputGovernanceDenied`
(with the sentinel's `audit_id`) when `result` is `RedactedOutput`, and returns
`result` unchanged otherwise — the uniform caller contract for code that handles
both read and write tools.

### No-echo (D6)

The output seam names the firing rail and a safe summary in every audit row and
log line. The raw matched text (the PII value, the attacker payload) is never
echoed into the audit record, the exception message, or the log. The sentinel
`str()` names the `audit_id` only.

### Tool I/O classification

An action absent from `tool_io_map` defaults to `write` (fail-closed). The
governor logs a one-time WARNING on the first call for each unmapped action so
a read tool the operator forgot to classify is caught in dev/staging, not
silently redacted in production.

### Non-JSON return values

A tool return that is not `str`, `bytes`, or a JSON-native tree (dict/list/
number/bool/None with string keys and at most 64 levels deep) is denied
fail-closed: `OutputGovernanceDenied` is raised for read tools; `RedactedOutput`
is returned for write tools. An unscreenable value is never passed through.
